...
Remote fetch rejects unsupported non-text content types and responses larger than the configured byte limit instead of silently ingesting truncated or binary content. These permanent fetch errors are not retried, while exhausted transient retries are reported with the number of attempts. The final URL after redirects is revalidated against source policy before ingestion.
